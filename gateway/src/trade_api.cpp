 ...
 ******************************************************************************
 */
#include "gateway/api/trade_api.h"
#include "zmq_impl/trade_api_zmq_impl.h"
...
namespace trade {
    ...
    TradeApi::TradeApi() {
        ...
        zmq_impl_.on_disconnected = [this](int reason) {
            if (spi_)
                spi_->OnFrontDisconnected(reason);
        };
    ...
    void TradeApi::Disconnect() { zmq_impl_.Disconnect(); }

    bool TradeApi::IsConnected() const { return zmq_impl_.IsConnected(); }
